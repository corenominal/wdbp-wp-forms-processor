...
function wdbp_wp_forms_admin( $request_data )
{
	...
	{
		case 'list_forms':
            $sql = "SELECT wdbp_wp_forms.id, name, wdbp_wp_forms.date_created, COUNT(form_id) AS submissions
                      FROM wdbp_wp_forms LEFT JOIN wdbp_wp_forms_submissions
                        ON wdbp_wp_forms.id = wdbp_wp_forms_submissions.form_id
                     GROUP BY wdbp_wp_forms.id";
            $sql .= " ORDER BY wdbp_wp_forms.id DESC LIMIT " . $data['limit'] . " OFFSET " . $data['offset'];
        ...
        case 'delete_form':
			$wpdb->delete( 'wdbp_wp_forms', array( 'id' => $data['id'] ), array( '%d' ) );
        ...
        case 'get_form':
			$sql = "SELECT * FROM wdbp_wp_forms WHERE id = " . $data['form'];
        ...
        case 'save_form':
            ...
            $wpdb->update( 'wdbp_wp_forms', // table
        ...
        case 'list_subs':
            ...
            $sql = "SELECT COUNT(*) AS `submissions` FROM `wdbp_wp_forms_submissions`";
            ...
            if( $data['search'] != '' )
            {
                ...
                $sql .= "wdbp_wp_forms_submissions.data LIKE '%" . $data['search'] . "%'";

            }

            $data['total'] = $wpdb->get_row( $sql, ARRAY_A );

            $sql = "SELECT wdbp_wp_forms_submissions.id, form_id, name, ip, wdbp_wp_forms_submissions.date_created
                    FROM wdbp_wp_forms_submissions
                    JOIN wdbp_wp_forms
                    ON wdbp_wp_forms_submissions.form_id = wdbp_wp_forms.id ";
            ...
            $sql .= " ORDER BY wdbp_wp_forms_submissions.id DESC LIMIT " . $data['limit'] . " OFFSET " . $data['offset'];
        ...
        case 'get_sub':
            $sql = "SELECT * FROM wdbp_wp_forms_submissions WHERE id = " . $data['id'];
        ...
        case 'remove_selected_subs':
            ...
            {
                $wpdb->delete( 'wdbp_wp_forms_submissions', array( 'id' => $sub ), array( '%d' ) );
            }
            $sql = "SELECT COUNT(*) AS `submissions` FROM `wdbp_wp_forms_submissions`";
            if( $data['form'] != 'all' )
            {
        ...
        case 'remove_all_subs':
            if( $data['form'] != 'all' )
            {
                $wpdb->delete( 'wdbp_wp_forms_submissions', array( 'form_id' => $data['form'] ), array( '%d' ) );
            }
            else
            {
