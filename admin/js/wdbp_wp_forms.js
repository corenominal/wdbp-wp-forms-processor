jQuery(document).ready(function($)
{
    ...
    function get_forms()
    {
        ...
        .done(function( data ) {
            ...
            {
                var offset = $( '#wdbp-wp-forms' ).data( 'offset' );
                var limit = $( '#wdbp-wp-forms' ).data( 'limit' );
                offset = offset - limit;
                $( '#wdbp-wp-forms' ).data( 'offset', offset );
                get_forms();
                return;
            }
            if( items > 0 )
            {
                ...
                $( '#wdbp-wp-forms' ).data( 'pages', pages );
                $( '#wdbp-wp-forms' ).data( 'total', items );
                var offset = $( '#wdbp-wp-forms' ).data( 'offset' );
			...
			else
			{
				$.each(data.forms, function(i, form)
				{
                    ...
                    {
                        subs = '<a href="admin.php?page=wdbp_wp_forms_submissions&name=' + form.name + '&form=' + form.id + '">' + form.submissions + '</a>';
                    }

                    var options = '<button data-id="' + form.id + '" class="button wdbp-wp-forms-edit-form-button">Edit</button> ';
                    options += '<button data-id="' + form.id + '" class="button wdbp-wp-forms-info-form-button">Info</button> ';
                    options += '<button data-id="' + form.id + '" class="button wdbp-wp-forms-remove-form-button">Remove</button>';
                    ...
                    forms += '<td class="wdbp-wp-forms-form-options-cell' + form.id + '">';
                    forms += options;
					forms += '</td>';
                    forms += '</tr>';
				});
        ...
        .fail(function( data ) {
            console.log( "error" );
            $( '#wdbp-wp-notify' ).html( '<div class="notice notice-error is-dismissible"><p>Error: data request failed. Try reloading the page.</p></div>' );
        });
    }
	 ...
	 * Edit form
	 */
	$( document ).on( 'click', '.wdbp-wp-forms-edit-form-button', function( e )
	{
		e.preventDefault();
		var id = $( this ).attr( 'data-id' );
        window.location.href = "admin.php?page=wdbp_wp_forms_edit&form=" + id;
	});

    /**
	 * Remove form from list
	 */
	$( document ).on( 'click', '.wdbp-wp-forms-remove-form-button', function( e )
	{
		e.preventDefault();
		var id = $( this ).attr( 'data-id' );
    ...
    $( document ).on( 'click', '.remove-form-prompt-yes', function( e )
	{
		...
		.fail(function()
		{
            ...
            $( '#wdbp-wp-notify' ).html( '<div class="notice notice-error is-dismissible"><p>Error: failed to remove form. Try reloading the page.</p></div>' );
		});
	});
	 ...
	 * Get form info
	 */
	$( document ).on( 'click', '.wdbp-wp-forms-info-form-button', function( e )
	{
        e.preventDefault();
		var id = $( this ).attr( 'data-id' );
        window.location.href = "admin.php?page=wdbp_wp_forms_info&form=" + id;
	});
    ...
    {
        e.preventDefault();
        var pages = $( '#wdbp-wp-forms' ).data( 'pages' );
        var limit = $( '#wdbp-wp-forms' ).data( 'limit' );
