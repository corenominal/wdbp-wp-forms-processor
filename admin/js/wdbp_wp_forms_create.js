jQuery(document).ready(function($){
    ...
    $( document ).on( 'submit', '#create-form-form', function( e )
	{
		...
		.done(function( data )
		{
            if( data.error)
            {
                $( '#wdbp-wp-notify' ).html( '<div class="notice notice-error is-dismissible"><p>Error: ' + data.error + '</p></div>' );
                $( '#create-form-form #submit' ).removeAttr( 'disabled' );
            }
            else
            {
                window.location.href = "admin.php?page=wdbp_wp_forms_edit&form=" + data.id;
            }
		})
		.fail(function()
		{
